...
if __name__=="__main__":
    ...
    build_file="test02.sbf"
    build_path = os.path.join(output_dir, build_file)
      
    if not os.path.exists(output_dir): os.mkdir(output_dir)
    sip_file = os.path.join(sip_files_dir, "test02.sip")
    ...
    makefile.extra_defines+=['SATPACAMWIDGET_LIBRARY','QT_CORE_LIB', 'QT_GUI_LIB', 'QT_WIDGETS_LIB']
    ...
    makefile.extra_libs+=['SatpaCamWidgetLib']
    ...
    os.chdir("src")    
    qmake_cmd=qmake_exe
    #  if sys.platform=="win32": qmake_cmd+=" -spec msvc2019"
    print()
    print(qmake_cmd)
    os.system(qmake_cmd)
    sys.exit()
